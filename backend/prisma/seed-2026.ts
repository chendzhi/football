 ...
 * Source: FIFA.com / OneFootball / multiple sports outlets
 *
 * 48 teams · 12 groups · 72 group stage matches · 104 total
 */
...
// ─── OFFICIAL GROUP STAGE SCHEDULE (times verified against sporttery.cn) ───
// Times in UTC. Beijing time = UTC + 8h.
// Pattern: 17:00/20:00/23:00/02:00 UTC = 01:00/04:00/07:00/10:00 Beijing (next day)
type M = [string, string, string, string, string, number?, number?];
const SCHEDULE: M[] = [
  // ===== MATCHDAY 1: June 11-17 (Beijing: 6/11 晚 – 6/18 上午) =====
  // Jun 11 (Thu) — Opening matches
  ['m01','2026-06-11T17:00:00Z','Group A','mex','rsa',2,0],
  ['m02','2026-06-11T20:00:00Z','Group A','kor','cze',2,1],
  // Jun 12 (Fri)
  ['m03','2026-06-12T17:00:00Z','Group B','can','bih',1,1],
  ['m04','2026-06-12T20:00:00Z','Group D','usa','par',4,1],
  // Jun 13 (Sat) — verified against Baidu sports
  ['m05','2026-06-12T19:00:00Z','Group B','qat','sui',1,1],   // 北京 03:00
  ['m06','2026-06-12T22:00:00Z','Group C','bra','mar',1,1],   // 北京 06:00
  ['m07','2026-06-13T01:00:00Z','Group C','hai','sco',0,1],   // 北京 09:00 (FIXED score)
  ['m08','2026-06-13T04:00:00Z','Group D','aus','tur',1,0],   // 北京 12:00 (LIVE!)
  // Jun 14 (Sun) — verified against Baidu sports
  ['m09','2026-06-13T17:00:00Z','Group E','ger','cuw'],       // 北京 01:00
  ['m10','2026-06-13T20:00:00Z','Group F','ned','jpn'],       // 北京 04:00
  ['m11','2026-06-13T23:00:00Z','Group E','civ','ecu'],       // 北京 07:00
  ['m12','2026-06-14T02:00:00Z','Group F','swe','tun'],       // 北京 10:00
  // Jun 15 (Mon) — verified
  ...
  // ===== MATCHDAY 3: June 24-27 (simultaneous kickoffs) =====
  // Jun 24 (Tue) — Groups A,B,C
...
// ─── MAIN ───
async function main() {
  console.log('⚽ 2026 FIFA World Cup — OFFICIAL SCHEDULE\n');
  ...
  // Matches
  console.log(`📅 ${SCHEDULE.length} group stage matches...`);
  ...
  for (const [mid, date, group, home, away, hs, as] of SCHEDULE) {
    ...
    await prisma.match.create({
      data: { id: mid, matchDate: new Date(date), groupName: group, stage: 'GROUP_STAGE', homeTeamId: home, awayTeamId: away, status: hasResult ? 'completed' : 'scheduled', homeScore: hs ?? null, awayScore: as ?? null },
    });

    // ELO-based odds
    const h = T[home]; const a = T[away];
    const eloDiff = h[3] - a[3];
    const homeStr = 1 / (1 + Math.exp(-eloDiff / 400));
    const drawP = Math.max(0.15, 0.28 - Math.abs(eloDiff) / 2500);
    const homeP = homeStr * (1 - drawP); const awayP = (1 - homeStr) * (1 - drawP);
    const mgn = 1.07;

    await prisma.odds.create({
      data: { id: `o_${mid}`, matchId: mid, currentHomeOdds: parseFloat((mgn / homeP).toFixed(2)), currentDrawOdds: parseFloat((mgn / drawP).toFixed(2)), currentAwayOdds: parseFloat((mgn / awayP).toFixed(2)) },
    });
    ...
    if (hasResult) {
      ...
      await prisma.predictionHistory.create({
        data: { id: `ph_${mid}`, matchId: mid, teamId: home, predHomeWin: parseFloat(homeP.toFixed(4)), predDraw: parseFloat(drawP.toFixed(4)), predAwayWin: parseFloat(awayP.toFixed(4)), actualOutcome: outcome, featureVersion: 'official_2026', modelVersion: 'lambda_v1.0', simulationVersion: 'simulation_v1.0' },
      });
    } else {
      scheduled++;
    }
  ...
  console.log(`   ${completed} completed (REAL) · ${scheduled} to predict`);
  console.log(`\n✅ 2026 World Cup — OFFICIAL FIFA Schedule`);
}
