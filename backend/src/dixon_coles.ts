/**
 * Dixon-Coles v3 — Stronger Low-Score Correction
 *
 * rho = -0.25 (stronger: boosts 0-0, 1-1 draws significantly)
...
// Football has ρ ≈ -0.05 to -0.25 (negative = low scores positively correlated)
// With ρ < 0: τ(0,0)>1, τ(1,1)>1 → low-score draws more likely
export function dixonColesTau(x: number, y: number, lambda: number, mu: number, rho = -0.25): number {
...
export function dixonColesMaxTau(lambda: number, mu: number, rho = -0.25): number {
...
export function lowScoreWeight(score: string, lambda: number, mu: number): number {
  ...
  switch (score) {
    case '0-0': return 1 + tau * 1.5;   // boost 0-0 draws
    case '1-0': case '0-1': return 1 + tau * 0.8;
    case '1-1': return 1 + tau * 1.2;   // boost 1-1 draws
    default: return 1.0;
  }
}
